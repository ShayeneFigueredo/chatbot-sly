...
# Mock do módulo buscador_temas para testes controlados
import buscador_temas

# Mock da função buscar_tema
buscar_tema_original = buscador_temas.buscar_tema
...
class ChatbotTester:
    ...
    def run_test(self, name, inputs, checks):
        ...
        with patch("builtins.print", self.mock_print), \
             ...
             patch("chatbot_sly.cliente", self.mock_groq), \
             patch("buscador_temas.buscar_tema", mock_buscar_tema):
            try:
                # Importa e executa
                import chatbot_sly

                # Simula o loop principal
                self._run_main_loop(chatbot_sly)
            except SystemExit:
                pass
    ...
    def _run_main_loop(self, chatbot_sly):
        """Executa o loop principal do chatbot."""
        # Inicialização
        msg, botoes = chatbot_sly.mensagem_boas_vindas()
        self.prints.append(f"🤖 Sly:\n{msg}")
        chatbot_sly.mostrar_botoes(botoes, "\n  💬 Ou digite sua pergunta livremente (a Sly entende!)\n  [0] Sair")

        historico = [{"role": "system", "content": chatbot_sly.SYSTEM_PROMPT}]
        tela_atual = "menu"
        dados_pedido = {}
        editando = False
        ultimos_botoes = []
        ...
        while not self.exit_requested:
            ...
            if escolha == "0":
                msg, botoes = chatbot_sly.mensagem_cancelamento()
                self.prints.append(f"🤖 Sly:\n{msg}")
                break
            ...
            if escolha == "1" and tela_atual == "menu":
                ...
                msg, botoes = chatbot_sly.resposta_ver_slides_prontos()
                self.prints.append(f"🤖 Sly:\n{msg}")
            ...
            if escolha == "2" and tela_atual == "menu":
                ...
                dados_pedido = {}
                msg, botoes = chatbot_sly.iniciar_pedido_personalizado()
                self.prints.append(f"🤖 Sly:\n{msg}")
                chatbot_sly.mostrar_botoes(botoes)
                continue
            ...
            if escolha == "4" and tela_atual == "menu":
                tela_atual = "precos"
                msg, botoes = chatbot_sly.resposta_quanto_custa()
                self.prints.append(f"🤖 Sly:\n{msg}")
                chatbot_sly.mostrar_botoes(botoes, "\n  💬 Ou digite sua pergunta livremente")
                continue
            ...
            if tela_atual == "duvida":
                ...
                chatbot_sly.mostrar_botoes(botoes_pd, "  [0] Sair")
            ...
            if tela_atual == "slides_prontos":
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, ["🔍 Buscar um tema", "🔙 Voltar ao menu"])
            ...
            if tela_atual == "pedido_tema":
                ...
                if editando:
                    editando = False
                    tela_atual = "resumo"
                    msg, botoes = chatbot_sly.mostrar_resumo(dados_pedido)
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
                else:
                    tela_atual = "pedido_modelo"
                    msg, botoes = chatbot_sly.perguntar_modelo()
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
                continue
            ...
            if tela_atual == "pedido_modelo":
                ...
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, botoes_modelo)

                if "Diferença" in escolha_resolvida or "diferença" in escolha.lower():
                    msg, botoes = chatbot_sly.explicar_diferenca_canva_powerpoint()
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
                ...
                if escolha_resolvida in botoes_modelo[:5]:
                    ...
                    if "Temas" in escolha_resolvida:
                        ...
                        msg, botoes = chatbot_sly.perguntar_qual_tema()
                        self.prints.append(f"🤖 Sly:\n{msg}")
                        chatbot_sly.mostrar_botoes(botoes)
                    else:
                        tela_atual = "pedido_prazo"
                        msg, botoes = chatbot_sly.perguntar_prazo()
                        self.prints.append(f"🤖 Sly:\n{msg}")
                        chatbot_sly.mostrar_botoes(botoes)
            ...
            if tela_atual == "pedido_canva_ppt":
                ...
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, botoes_cp)
            ...
            if tela_atual == "pedido_prazo":
                ...
                if editando:
                    ...
                    msg, botoes = chatbot_sly.perguntar_nomes()
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
                continue
            ...
            if tela_atual == "pedido_nomes":
                ...
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, botoes_nomes)
                ...
                if editando:
                    ...
                    msg, botoes = chatbot_sly.perguntar_observacoes()
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
                continue
            ...
            if tela_atual == "pedido_obs":
                ...
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, botoes_obs)
                ...
                if escolha_resolvida == "✅ Finalizar Pedido":
                    ...
                    if dados_pedido["observacoes"]:
                        ...
                        dados_pedido["observacoes"] = escolha
                    msg, botoes = chatbot_sly.confirmar_recebido()
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
                continue
            ...
            if tela_atual == "resumo":
                ...
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, botoes_resumo)
                if escolha_resolvida == "✅ Confirmar pedido":
                    tela_atual = "pagamento"
                    msg, botoes = chatbot_sly.instrucoes_pagamento(dados_pedido)
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
                elif escolha_resolvida == "✏️ Quero mudar algo":
                    tela_atual = "mudar"
                    msg, botoes = chatbot_sly.perguntar_o_que_mudar()
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
            ...
            if tela_atual == "mudar":
                ...
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, botoes_mudar)
            ...
            if tela_atual == "pagamento":
                ...
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, botoes_pag)
            ...
            if tela_atual == "aguardando_comprovante":
                if escolha.lower() in ("voltar", "menu", "🔙 voltar ao menu"):
                    ...
                    msg, botoes = chatbot_sly.comprovante_recebido()
                    self.prints.append(f"🤖 Sly:\n{msg}")
                    chatbot_sly.mostrar_botoes(botoes)
            ...
            if tela_atual == "menu":
                ...
                if tema_encontrado:
                    ...
                    chatbot_sly.mostrar_botoes(["📱 Canva", "💻 PowerPoint", "🔙 Voltar ao menu"])
                    continue

            # Pós-dúvida
            if tela_atual == "pos_duvida":
                botoes_pd = ["💬 Continuar dúvida", "🔙 Voltar ao menu"]
                escolha_resolvida = chatbot_sly.resolver_botao(escolha, botoes_pd)
...
def run_all_tests():
    ...
    results.append(tester.run_test(
        ...
        inputs=["2"],
        checks=[
            check_output_contains("Qual o ASSUNTO"),
    ...
    results.append(tester.run_test(
        ...
        checks=[
            check_output_contains("Esperamos trabalhar com você"),
        ]
    ))
